...
class PaletteEditor:
    ...
    def __init__(self):
        self.genesis_palette_locations = [
            0x20000, 0x30000, 0x40000, 0x50000, 0x60000, 0x70000  # Common palette areas
        ]
    ...
    def apply_palette_modification(self, rom_data: bytes, palette_index: int, new_palette: List[Tuple[int, int, int]]) -> bytes:
...
class TextEditor:
    ...
    def replace_text_string(self, rom_data: bytes, offset: int, old_text: str, new_text: str) -> bytes:
        """Replace a text string at a specific offset"""
        if len(new_text) > len(old_text):
            raise ValueError(f"New text is too long ({len(new_text)} > {len(old_text)})")
        ...
        modified_data[offset:offset + len(old_bytes)] = new_bytes + b'\x00' * (len(old_bytes) - len(new_bytes))
            ...
            if 0 <= palette_index < len(palettes):
                ...
                with open(output_path, 'w') as f:
                    json.dump({
                        ...
                        'palette_data': [[(r,g,b) for r,g,b in p] for p in palettes[:10]]
                    }, f, indent=2)
